...
export default async function RuSearchPage({ searchParams }: { searchParams: { q?: string } }) {
			...
			<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginTop: 12, alignItems: 'start' }}>
				<section style={{ textAlign: 'center' }}>
					...
					<div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center' }}>
						{ai.length ? (
							ai.map(item => (
								<a key={`${item.type}-${item.id}`} href={item.type === 'news' ? `/ru/news/${item.id}` : `/ru/columns/${item.id}`} className="card" style={{ width: '100%' }}>
									{item.resolved_image && (
										<div style={{ marginBottom: 8 }}>
											<img src={item.resolved_image} alt="" className="thumb" />
										</div>
									)}
									<div className="card-title">{item.title_ru || item.title}</div>
									{item.snippet && (
